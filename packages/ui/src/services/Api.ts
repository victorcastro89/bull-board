import {AppJob, JobCleanStatus, JobRetryStatus, RedisStats, Status,} from '@bull-board/api/typings/app';
import {GetJobResponse, GetMetricsResponse, GetQueuesResponse} from '@bull-board/api/typings/responses';
...
export class Api {
    ...
    public getQueueStats({start, end, timeframe}: {
        start?: number,
        end?: number
        timeframe: string
    }): Promise<GetMetricsResponse> {
        return this.axios.get(`/queues/stats`, {params: {timeframe, start, end}});
    }
    ...
    public getMetrics(queueName: string, jobId: AppJob['id']): Promise<GetJobResponse> {
        return this.axios.get(
            `/queues/${encodeURIComponent(queueName)}/${encodeURIComponent(`${jobId}`)}`
        );
    }
